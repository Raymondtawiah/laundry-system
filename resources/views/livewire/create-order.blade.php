<div>
    <form method="POST" action="{{ route('orders.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <select name="customer_id" class="w-full rounded-lg border-zinc-600 bg-zinc-700 text-white focus:border-blue-500 focus:ring-blue-500" required>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="delivery_type" class="w-full rounded-lg border-zinc-600 bg-zinc-700 text-white focus:border-blue-500 focus:ring-blue-500" required>
            ...
            <div>
                ...
                <select name="service_type" class="w-full rounded-lg border-zinc-600 bg-zinc-700 text-white focus:border-blue-500 focus:ring-blue-500" required>
            ...
            @else
                <div class="space-y-3">
                    @foreach($orderItems as $index => $orderItem)
                        <div class="flex items-center gap-3 p-4 bg-zinc-700/50 rounded-lg border border-zinc-600">
                            <div class="flex-1">
                                <select 
                                    wire:model="orderItems.{{ $index }}.item_id" 
                                    wire:change="onItemChange({{ $index }})"
                                    class="w-full rounded-lg border-zinc-600 bg-zinc-700 text-white text-sm"
                                    required
                                >
                            ...
                            <div class="w-20 text-center text-sm text-zinc-400">
                            ...
                            <div class="w-24">
                                <input 
                                    ...
                                    min="1" 
                                    class="w-full rounded-lg border-zinc-600 bg-zinc-700 text-white text-sm text-center"
                                    required
                                >
                            </div>
                            <div class="w-28 text-right text-sm font-medium text-white">
                                GH₵{{ number_format($orderItem['subtotal'], 2) }}
                            </div>
                            <button type="button" wire:click="removeItem({{ $index }})" class="text-red-400 hover:text-red-300">
